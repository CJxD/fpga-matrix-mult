...
#define PrintOperator(T) static inline superstl::ostream& operator <<(superstl::ostream& os, const T& obj) { return obj.print(os); }
...
#define DeclareStringBufToStream(T) inline ostream& operator <<(ostream& os, const T& arg) { stringbuf sb; sb << arg; os << sb; return os; }
  ...
  struct substring {
    ...
    substring(const char* str, int start, int length) {
      int r = strlen(str);
      this->length = ptlsim_min(length, r - start);
      this->str = str + ptlsim_min(start, r);
    }
  };
  ...
  static inline T* renew(T* p, size_t oldcount, size_t newcount) {
    ...
    if unlikely (!p) ptlsim_assert(oldcount == 0);
  ...
  struct ObjectLinkManager {
    ...
    //   return baseof(T, hashlink, link); // a.k.a. (T*)((byte*)link) - ptlsimoffsetof(T, hashlink);
    // }
    //
...
#undef __builtin_clzl
  ...
  struct BitmapAllocator3Level {
    ...
    void reset(size_t sizelimit = N) {
      ptlsim_assert(sizelimit >= 4096);
      ptlsim_assert(sizelimit <= 262144);
      // size must be a multiple of 64*64 = 4096
      ptlsim_assert(lowbits(sizelimit, 12) == 0);
    ...
    W64s alloc(W64 index) {
      ...
      //ptlsim_assert(level1[slot1][offset1]); // make sure it's currently free
      ...
      highest_count = ptlsim_max(highest_count, int(index+1));
...
#define foreachlink(list, type, iter) \
  ...
  struct SearchableChunkList16Entry {
    ...
    T* add(const T& e) {
      ...
      ptlsim_assert(index < lengthof(list));
  ...
  void subtract_structures(S& d, const S& a, const S& b) {
    ...
    T* dp = (T*)&d;
    ptlsim_assert((sizeof(S) % sizeof(T)) == 0);
  ...
  struct RecursiveMutex {
    ...
    void release() {
      W16s current = current_vcpuid();
      ptlsim_assert(locking_vcpuid == current);
      ptlsim_assert(counter > 0);
