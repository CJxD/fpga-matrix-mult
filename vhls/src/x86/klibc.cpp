...
int format_float(char* buf, int bufsize, double v, int precision, int pad) {
  ...
  remaining = ptlsim_max(remaining - n, 0);
  total += n;
  if (remaining < 1) return total;
  ptlsim_assert(n < bufsize-1);
  buf[n] = '.';
  total++;
  remaining--;
  n = format_integer(buf + n + 1, ptlsim_max(ptlsim_min(remaining, precision+1), 0), fracint, 0, 0);
  total += n;
  buf[total] = 0;
  return total;
}
