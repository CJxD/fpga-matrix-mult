...
inline int Context::copy_from_user(void* target, Waddr addr, int bytes, PageFaultErrorCode& pfec, Waddr& faultaddr, bool forexec, Level1PTE& ptelo, Level1PTE& ptehi) {
  ...
  n = ptlsim_min((Waddr)(4096 - lowbits(addr, 12)), (Waddr)bytes);
...
inline int Context::copy_to_user(Waddr target, void* source, int bytes, PageFaultErrorCode& pfec, Waddr& faultaddr) {
  ...
  int nlo = ptlsim_min((Waddr)(4096 - lowbits(target, 12)), (Waddr)bytes);
...
static inline W64 storemask(Waddr addr, W64 data, byte bytemask) {
  ...
  // MP 22May13 - this is writing to a memory location denoted
  // by addr, we probably want to change this to use our
  // memory infrastructure?
  superstl::cerr << "addr to write is 0x" << hexstring(addr, 64) << " and data is 0x" << hexstring(data, 64) << superstl::endl;
  superstl::cerr << "bytemask is 0x" << hexstring(bytemask, 64) << superstl::endl;
  ptlsim_assert(0);
