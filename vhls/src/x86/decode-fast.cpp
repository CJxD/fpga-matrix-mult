...
bool TraceDecoder::decode_fast() {
  ...
  case 0x90: {
    ...
    int uop = (prefixes == PFX_REPZ) ? OP_pause:OP_nop; // DJG
    EndOfDecode();
    this << TransOp(uop, REG_temp0, REG_zero, REG_zero, REG_zero, 3);
    break;
  }
  ...
  case 0x140 ... 0x14f: {
    ...
    if (ra.type == OPTYPE_REG) {
      ...
      ptlsim_assert(ra.type == OPTYPE_MEM);
      prefixes &= ~PFX_LOCK;
    ...
    ptlsim_assert(condreg != REG_zero);
  ...
  case 0x190 ... 0x19f: {
    ...
    if (rd.type == OPTYPE_REG) {
      ...
      ptlsim_assert(rd.type == OPTYPE_MEM);
      r = REG_temp7;
    }
  ...
  case 0x1bb: { // btc ra,rb  101 11 011
    ...
    ptlsim_assert(rd.type == OPTYPE_REG);
    int rdreg = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
    int rareg = arch_pseudo_reg_to_arch_reg[ra.reg.reg];
