...
namespace superstl {
  ...
  int odstream::write(const void* data, int count) {
    ...
    while (count) {
      int n = ptlsim_min(bufsize - tail, count);
      memcpy(buf + tail, p, n);
      tail += n;
      ptlsim_assert(inrange(tail, 0, bufsize));
      count -= n;
      total += n;
      p += n;
      if unlikely (!n) {
  ...
  void stringbuf::reset(int length) {
    ...
    length = ptlsim_max(length, stringbuf_smallbufsize);
    p = buf;
    this->length = length;
    p[0] = 0;
  }
  ...
  void stringbuf::resize(int newlength) {
    if (newlength > length) {
      ...
      if (buf == smallbuf) {
        ...
        ptlsim_assert(strlength < newlength);
  ...
  stringbuf& operator <<(stringbuf& os, const intstring& is) {
    ...
    int reqbytes = ptlsim_max(len, width) + 1;
    os.reserve(reqbytes);

    if (leftalign) {
      memcpy(os.p, buf, len);
      os.p += len;
      width = ptlsim_max(width - len, 0);
      if(width > 0)
	memset(os.p, ' ', width);
      os.p += width;
    } else {
  ...
  int idstream::fillbuf() {
    ...
    if (head < tail) {
      ...
      if (bufused) {
        ...
        ptlsim_assert(bufused == bufsize);
      } else {
        ptlsim_assert(bufused == 0);
        // buffer empty
        int bytes = bufsize;
    ...
    ptlsim_assert(inrange(head, 0, bufsize-1));
    ptlsim_assert(inrange(tail, 0, bufsize-1));
    ptlsim_assert(inrange(bufused, 0, bufsize));

    return 1;
  }

  int idstream::unread(int bytes) {
    ptlsim_assert(bytes <= bufused);
  ...
  int idstream::readbuf(byte* dest, int count) {
    ptlsim_assert(count <= bufsize);
    int n;
    if (head < tail) {
      // ....|||||||.....
      //     h      t
      // 0123456789abcdef
      n = ptlsim_min(bufused, count);
      ...
      //     t      h
      // 0123456789abcdef
      n = ptlsim_min(ptlsim_min(bufsize - head, count), bufused);
      memcpy(dest, &buf[head], n);
      head = addmod(head, n);
      bufused -= n;
    }
  ...
  int idstream::read(void* dest, int count) {
    ...
    while (count) {
      fillbuf();
      int n = readbuf(p, ptlsim_min(count, bufsize));
      if (!n) {
        // end of stream
        error = 1;
        return r;
...
#define decl_div_rem_s(T) template bool div_rem_s(T& quotient, T& remainder, T dividend_hi, T dividend_lo, T divisor);
  ...
  struct ExitCallbacks {
    ExitCallbacks(bool x) { ptlsim_assert(atexit(close_streams) == 0); }
  };
...
int format_integer(char* buf, int bufsize, W64s v, int size, int flags, int base, int precision) {
  ...
  char* end = format_number(buf, buf + bufsize - 2, v, base, ptlsim_min(bufsize-1, size), precision, flags);
  // null terminate
  *end = 0;
  return (end - buf);
}
