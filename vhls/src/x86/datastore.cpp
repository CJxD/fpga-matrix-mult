...
void DataStoreNode::rename(const char* newname) {
  ...
  if (oldparent)
    ptlsim_assert(oldparent->remove(name));
...
DataStoreNode::~DataStoreNode() {
  if (parent)
    ptlsim_assert(parent->remove(name));
  cleanup();
  removeall();
  if (subnodes)
    delete subnodes;
...
void DataStoreNode::removeall() {
  ...
  if (subnodes) ptlsim_assert(subnodes->count == 0);
...
DataStoreNode::operator W64s*() const {
  ptlsim_assert(type == DS_NODE_TYPE_INT);
...
DataStoreNode::operator double*() const {
  ptlsim_assert(type == DS_NODE_TYPE_FLOAT);
...
const char* DataStoreNode::string() const {
  ptlsim_assert(type == DS_NODE_TYPE_STRING);
  return getdata().s;
}
    ...
    if (count == 1) {
      ...
      if (histogramarray) {
        ...
        foreach (i, count) {
          ...
          minvalue = ptlsim_min((W64)values[i].w, minvalue);
          maxvalue = ptlsim_max((W64)values[i].w, maxvalue);
        }

        double average = double(weightedsum) / double(total);
        W64 thresh = ptlsim_max((W64)math::ceil((double)total * printinfo.histogram_thresh), (W64)1);
        W64 base = histomin;
        int width = digits(ptlsim_max(histomin, histomax));
        int valuewidth = digits(maxvalue);
        int w = ptlsim_max(width, valuewidth);
...
bool DataStoreNode::read(idstream& is) {
  ...
  if (h.magic != DSN_MAGIC_VER_3) {
    superstl::cerr << "DataStoreNode::read(): ERROR: stream does not have proper DSN version 2 header (0x", 
...
odstream& DataStoreNodeTemplate::write(odstream& os) const {
  ...
  ptlsim_assert(subcount == subnodes.length);
...
DataStoreNodeTemplate::DataStoreNodeTemplate(idstream& is) {
  ...
  ptlsim_assert(magic == DataStoreNodeTemplateBase::MAGIC);
  ptlsim_assert(length == sizeof(DataStoreNodeTemplateBase));
...
DataStoreNode* DataStoreNodeTemplate::reconstruct(const W64*& p) const {
  ...
  case DS_NODE_TYPE_STRING: {
    if (count > 1) {
      ptlsim_assert(false); // not supported
...
void DataStoreNodeTemplate::subtract(W64*& p, W64*& psub) const {
  ...
  case DS_NODE_TYPE_STRING: {
    ptlsim_assert(count == 1);
    ptlsim_assert((limit % 8) == 0);
    p += (limit / 8);
    psub += (limit / 8);
    break;
  }
...
void StatsFileWriter::flush() {
  ...
  ptlsim_assert(header.index_offset == (header.record_offset + (header.record_count * header.record_size)));
  ...
  ptlsim_assert(n == header.index_count);
...
bool StatsFileReader::open(const char* filename) {
  ...
  if (!is) {
    superstl::cerr << "StatsFileReader: cannot open ", filename, endl;
    return false;
  }
  ...
  if (!is) {
    superstl::cerr << "StatsFileReader: error reading header", endl;
    close();
    return false;
  }

  if (header.magic != StatsFileHeader::MAGIC) {
    superstl::cerr << "StatsFileReader: header magic or version mismatch", endl;
    close();
    return false;
  }
  ...
  if ((!is) | (!dst)) {
    superstl::cerr << "StatsFileReader: error while reading and parsing template", endl;
    close();
    return false;
  }
