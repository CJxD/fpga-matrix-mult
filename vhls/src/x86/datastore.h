...
struct DataStoreNode {
  ...
  superstl::ostream& print(superstl::ostream& os, const DataStoreNodePrintSettings& printinfo = DataStoreNodePrintSettings(), int depth = 0, double supersum = 0) const;
  ...
  superstl::ostream& generate_structural_code(superstl::ostream& os, int level = 0) const;
  superstl::ostream& generate_reconstruction_code(superstl::ostream& os, int level = 0) const;
  ...
  static DataStoreNode* apply(const F& func, const DataStoreNode& a, const DataStoreNode& b) {
    ...
    if (!((a.type == b.type) & (a.count == b.count))) {
      superstl::cerr << "DataStoreNode::apply(", a.name, ", ", b.name, "): mismatch types (", a.type, " vs ", b.type, "), count (", a.count, " vs ", b.count, ")", superstl::endl, superstl::flush;
      ptlsim_assert(false);
    }
    ...
    if (alist.length != blist.length) {
      superstl::cerr << "DataStoreNode::apply(", a.name, ", ", b.name, "): mismatch in subnode list length (", alist.length, " vs ", blist.length, ")", superstl::endl, superstl::flush;
      ptlsim_assert(alist.length == blist.length);
    }

    foreach (i, alist.length) {
      DataStoreNode& anode = *a.search(alist[i].key);
      DataStoreNode& bnode = *b.search(blist[i].key);
      ptlsim_assert(&anode); ptlsim_assert(&bnode);
  ...
  DataStoreNode& apply(const F& func, const DataStoreNode& b) {
    if (!((type == b.type) & (count == b.count))) {
      superstl::cerr << "DataStoreNode::apply(", name, ", ", b.name, "): mismatch types (", type, " vs ", b.type, "), count (", count, " vs ", b.count, ")", superstl::endl, superstl::flush;
      ptlsim_assert(false);
    }
    ...
    if (alist.length != blist.length) {
      superstl::cerr << "DataStoreNode::apply(", name, ", ", b.name, "): mismatch in subnode list length (", alist.length, " vs ", blist.length, ")", superstl::endl, superstl::flush;
      ptlsim_assert(alist.length == blist.length);
    }
...
struct DataStoreNodeTemplate: public DataStoreNodeTemplateBase {
  ...
  //
  superstl::ostream& generate_struct_def(superstl::ostream& os, int depth = 0) const;
...
static inline superstl::ostream& operator <<(superstl::ostream& os, const DataStoreNodeTemplate& node) {
...
static inline superstl::ostream& print(superstl::ostream& os, const StatsFileReader& reader) {
