...
void ptl_mm_flush_logging() {
  ...
  ptlsim_assert(sys_write(mm_logging_fd, mm_event_buffer_head, bytes) == bytes);
...
struct ExtentAllocator {
  ...
  int extent_size_to_slot(size_t size) const {
    return ptlsim_min(size-1, (size_t)(SIZESLOTS-1));
  }
  ...
  void* alloc(size_t size) {
    ...
    peak_bytes_allocated = ptlsim_max(peak_bytes_allocated, current_bytes_allocated);
  ...
  void free(void* p, size_t size) {
    ...
    frees++;
    current_bytes_allocated = ptlsim_min(current_bytes_allocated - (size * CHUNKSIZE), 0ULL);
  ...
  bool fast_validate() {
    ...
    foreach (i, SIZESLOTS) {
      ...
      ptlsim_assert(is_inside_ptlsim(r));

      while (r) {
        object_of_interest = r;
        ptlsim_assert(is_inside_ptlsim(r));
        ptlsim_assert(is_inside_ptlsim(r->sizelink.prev));
        ptlsim_assert(is_inside_ptlsim_heap(r->sizelink.next));
        ptlsim_assert(is_inside_ptlsim(r->startaddrlink.prev));
        ptlsim_assert(is_inside_ptlsim_heap(r->startaddrlink.next));
        ptlsim_assert(is_inside_ptlsim(r->endaddrlink.prev));
        ptlsim_assert(is_inside_ptlsim_heap(r->endaddrlink.next));
    ...
    foreach (i, HASHSLOTS) {
      ...
      while (link) {
        ptlsim_assert(is_inside_ptlsim(link));
        ptlsim_assert(is_inside_ptlsim(link->prev));
        ptlsim_assert(is_inside_ptlsim_heap(link->next));
        link = link->next;
      }
  ...
  bool full_validate() {
    ...
    foreach (i, SIZESLOTS) {
      ...
      ptlsim_assert(is_inside_ptlsim_heap(r));
  ...
  ostream& print(ostream& os) const {
    ...
    foreach (i, SIZESLOTS) {
      ...
      while (r) {
        ...
        ptlsim_assert(bytes >= min_expected_bytes);
        total_bytes += bytes;
...
struct SlabAllocator {
  ...
  struct PageHeader {
    ...
    bool validate() {
      ...
      while (link) {
        ptlsim_assert(inrange(Waddr(link), floor(Waddr(this), 4096), floor(Waddr(this), 4096) + 4095));
        ptlsim_assert(((Waddr(link) - floor(Waddr(this), 4096)) % objsize) == 0);
        ptlsim_assert(is_inside_ptlsim_heap(link));
        ptlsim_assert(is_inside_ptlsim_heap(link->prev));
        ptlsim_assert(is_inside_ptlsim_heap(link->next));
        n++;
        link = link->next;
      }

      ptlsim_assert(n == freecount); 
      return true;
    }
  };
  ...
  FreeObjectHeader* alloc_from_page(PageHeader* page) {  
    ...
    obj->unlink();
    ptlsim_assert(page->freecount > 0);
    page->freecount--;
    ...
    peak_objs_allocated = ptlsim_max(current_objs_allocated, peak_objs_allocated);

    current_bytes_allocated += objsize;
    peak_bytes_allocated = ptlsim_max(current_bytes_allocated, peak_bytes_allocated);
  ...
  PageHeader* alloc_new_page() {
    ...
    peak_pages_allocated = ptlsim_max(current_pages_allocated, peak_pages_allocated);
    ...
    ptlsim_assert(pfn < PTL_PAGE_POOL_SIZE);
  ...
  void* alloc() {
    ...
    if likely (page == free_pages) {
      ...
      page->link.addto((selflistlink*&)partial_pages);
      free_page_count--;
      ptlsim_assert(free_page_count >= 0);
    }
  ...
  void free(void* p) {
    ...
    current_bytes_allocated -= ptlsim_min((W64)objsize, current_bytes_allocated);

    FreeObjectHeader* obj = (FreeObjectHeader*)p;
    PageHeader* page = PageHeader::headerof(p);

    // ptlsim_assert(((Waddr(p) - Waddr(page->getbase())) % objsize) == 0);
    ...
    ptlsim_assert(page->freecount <= max_objects_per_page);
    page->freecount++;
  ...
  ostream& print_page_chain(ostream& os, PageHeader* page) const {
    while (page) {
      ...
      while (obj) {
        ...
        ptlsim_assert(inrange(slot, 0, int(max_objects_per_page)));
  ...
  bool validate() {
    ...
    while (link) {
      ...
      ptlsim_assert(h->validate());
      ptlsim_assert(h->freecount == max_objects_per_page);
      link = link->next;
    }
    ...
    while (link) {
      ...
      PageHeader* h = (PageHeader*)link;
      ptlsim_assert(h->validate());
      ptlsim_assert(inrange(h->freecount, 1, max_objects_per_page-1));
      link = link->next;
    }
...
void* ptl_mm_try_alloc_private_pages(Waddr bytecount, int prot, Waddr base, void* caller) {
  ...
  if (addr) {
    ...
    pagealloc.peak_bytes_allocated = ptlsim_max(pagealloc.peak_bytes_allocated, pagealloc.current_bytes_allocated);
  }
...
void ptl_mm_free_private_pages(void* addr, Waddr bytecount) {
  ...
  pagealloc.frees++;
  pagealloc.current_bytes_allocated -= ptlsim_min(pagealloc.current_bytes_allocated, (W64)bytecount);
...
void ptl_mm_init(byte* heap_start, byte* heap_end) {
  ...
  /*
    ...
    ptlsim_assert(mm_event_buffer_size > 4096);
...
void* ptl_mm_alloc(size_t bytes, void* caller) {
  ...
  if likely ((bytes <= SLAB_ALLOC_LARGE_OBJ_THRESH)) {
    ...
    ptlsim_assert(slot < SLAB_ALLOC_SLOT_COUNT);
    ...
    if unlikely (!p) {
      ...
      Waddr pagebytes = ptlsim_max((Waddr)ceil(bytes, PAGE_SIZE), (Waddr)GEN_ALLOC_CHUNK_SIZE);
      //
      // We need the pages in the low 2 GB of the address space so we can use
...
void* ptl_mm_alloc_aligned(int alignbits) {
  ...
  ptlsim_assert(lowbits(Waddr(p), alignbits) == 0);
  ptl_mm_add_event(PTL_MM_EVENT_ALLOC, PTL_MM_POOL_SLAB, getcaller(), p, bytes, slot);
...
/*asmlinkage void* malloc(size_t size) throw () {
