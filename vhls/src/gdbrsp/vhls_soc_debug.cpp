...
//
// The binding for a core is done like this:
//       if (external_debug_port) external_debug_port->connect_ip_block(txt, Cores[i]->core);
//
// The external block is typically a SystemC module that implements the vhls_debug_comms interface using multiple inheritance.
// All of the Prazor cpu cores support this since they inherit CORSEISA_IF, but the number of debug commands they support may vary.
//
  ...
  //  vhls_debug_fabric_base(mname, "debug_port")
{
  ...
  TheRspServer = new GdbServerSC("TheRspServer", 
				 /*flash region - not used*/0, 0x10000, 
				 /*RSP port no*/9600)  ;
  TheRspServer->set_debugUnit(this);
}
