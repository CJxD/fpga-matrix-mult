...
void assist_x87_fld80(Context& ctx) {
  ...
  long double real_data;
  u8_t* real_data_ptr = (u8_t*)(&real_data);

  PageFaultErrorCode pfec;
  Waddr faultaddr;
  
  // Read first 64 bits, and then 16 bits
  u16_t data1 = 0;
  ctx.mycore->busaccess.eval_mem16(addr + 8, data1, NULL, ctx.mycore->lt_i_delay);
  
  u64_t data2 = 0;
  ctx.mycore->busaccess.eval_mem64(addr, data2, NULL, ctx.mycore->lt_i_delay);

  u8_t* ptr_data = (u8_t*)(&data1);
  for(int i = 0; i < 2; i++) {
    data[i] = ptr_data[1 - i];
    real_data_ptr[i + 8] = ptr_data[i];
  }

  ptr_data = (u8_t*)(&data2); 
  for(int i = 0; i < 8; i++) {
    data[i + 2] = ptr_data[7 - i];
    real_data_ptr[i] = ptr_data[i];
  }

  int bytes = sizeof(X87Reg); /*ctx.copy_from_user(data, addr, sizeof(X87Reg), pfec, faultaddr);*/
  ...
  W64& tos = ctx.commitarf[REG_fptos];
  tos = (tos - 8) & FP_STACK_MASK;
  ctx.fpstack[tos >> 3] = x87_fp_80bit_to_64bit_2(real_data);
  setbit(ctx.commitarf[REG_fptags], tos);
  ctx.commitarf[REG_rip] = ctx.commitarf[REG_nextrip];
}
...
void assist_x87_fstp80(Context& ctx) {
  ...
  ctx.mycore->busaccess.set_mem64(addr, data1, NULL, ctx.mycore->lt_d_delay, false);
  ctx.mycore->busaccess.set_mem16(addr + 8, data2, NULL, ctx.mycore->lt_d_delay);

  int bytes = sizeof(X87Reg); /*ctx.copy_to_user(addr, data, sizeof(X87Reg), pfec, faultaddr);*/
...
void check_warned_about_x87() {
  ...
  if (warned_about_x87 == 16) {
    ...
    sb << superstl::endl,
      "//", superstl::endl,
      "// NOTE: This program is using a lot of legacy x87 floating point", superstl::endl, 

      // Use sc_time_stamp() here?
      //      "// at ", total_user_insns_committed, " commits, ", sim_cycle, " cycles.", superstl::endl,

      "// PTLsim executes x87 code very sub-optimally: it is HIGHLY recommended", superstl::endl,
      "// that you recompile the program with SSE/SSE2 support and/or update", superstl::endl,
      "// the standard libraries (libc, libm) to an SSE/SSE2-specific version.", superstl::endl, 
      "//", superstl::endl, superstl::endl;
    logfile << sb;
    superstl::cerr << sb;
  }
}
...
bool TraceDecoder::decode_x87() {
  ...
  case 0x615: { // fldCONST
    if (modrm.mod == 0x3) {
      ...
      TransOp stp(OP_st, REG_mem, REG_ctx, REG_imm, REG_temp1, 1, ptlsim_offsetof(Context, fpcw)); stp.internal = 1; this << stp;
    }
    break;
  }
  ...
  case 0x617: { // fnstcw | fprem fyl2xp1 fsqrt fsincos frndint fscale fsin fcos
    if (modrm.mod == 3) {
      ...
      DECODE(eform, rd, w_mode);
      EndOfDecode();
      TransOp ldp(OP_ld, REG_temp1, REG_ctx, REG_imm, REG_zero, 1, ptlsim_offsetof(Context, fpcw)); ldp.internal = 1; this << ldp;
