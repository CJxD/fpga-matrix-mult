...
/*** Finite automaton for instruction decoding building code ***/
...
unsigned long *or32_automaton = 0;
int nuncovered;
int curpass = 0;
...
/* Recursive utility function used to find best match and to build or32_automaton.  */
...
cover_insn (unsigned long *cur, int pass, unsigned int mask)
{
  ...
  if (ninstr == 1)
      ...
      for (i = 0; i < (1 << (unsigned long) best_len); i++)
	{
	  ...
	  if (c)
	    {
	      or32_debug (8, "%i> #%X -> %u\n", next - or32_automaton, i,
		     cur - or32_automaton);
	      *next = cur - or32_automaton;
	      cur = c;
	    }
	  else
	    {
...
/* Constructs new or32_automaton based on or32_opcodes array.  */
...
void build_automaton()
{
...
#ifdef HAVE_EXECUTION
  //printf ("Building or32_automaton... ");
#endif
  ...
  end = cover_insn (or32_automaton, curpass, 0xFFFFFFFF);
  if (end - or32_automaton > MAX_AUTOMATA_SIZE)
    {
      fprintf (stderr, "Automaton too large. Increase MAX_AUTOMATA_SIZE.");
      exit (1);
    }
#ifdef HAVE_EXECUTION
...
{
  unsigned long *a = or32_automaton;
  int i;
  while (!(*a & LEAF_FLAG))
    {
      unsigned int first = *a;
      //debug(9, "%i ", a - or32_automaton);
      a++;
      i = (insn >> first) & *a;
      a++;
      if (!*(a + i))
  ...
  //printf("Valid insn %i ins=%X mask=%X masked=%X base=%X\n", i, insn, ti[i].insn_mask, ti[i].insn_mask & insn, ti[i].insn);
  int d = ti[i].insn_mask & insn;
  if (d != ti[i].insn) i = -1;
  return i;
