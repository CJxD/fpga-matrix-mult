...
void uop_impl_pause(IssueState& state, W64 ra, W64 rb, W64 rc, W16 raflags, W16 rbflags, W16 rcflags) {
  //This is the same as a nop but we trap for this in the main decoder.
  state.reg.rddata = 0;
  state.reg.rdflags = 0;
  //printf("Doing pause\n");
  capture_uop_context(state, ra, rb, rc, raflags, rbflags, rcflags, OP_pause, 0);
}
...
make_x86_aluop_all_sizes(add, add, ZAPS|CF|OF, PRETEXT_ALL_FLAGS_IN);
make_x86_aluop_all_sizes(sub, sub, ZAPS|CF|OF, PRETEXT_ALL_FLAGS_IN);
...
make_exp_shiftop_64bit(rotcl, (ptlsim_assert(false), rd)); // not supported in 32-bit mode because it's too complex
make_exp_shiftop_64bit(rotcr, (ptlsim_assert(false), rd)); // not supported in 32-bit mode because it's too complex
#endif
...
void uop_impl_min_max(IssueState& state, W64 ra, W64 rb, W64 rc, W16 raflags, W16 rbflags, W16 rcflags) {
  T a = ra;
  T b = rb;
  T z = (compare_for_max) ? ptlsim_max(a, b) : ptlsim_min(a, b);
